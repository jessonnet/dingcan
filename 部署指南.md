...
# 执行数据库初始化脚本
mysql -u root -p canteen_ordering_system < sql/init.sql
```
...
```
# 本地使用密码重置接口（将所有用户密码重置为 123456）
curl http://localhost:8080/api/auth/reset-passwords -X 
POST

# 远程服务器使用
curl http://YOUR_IP/api/auth/reset-passwords -X 
POST
