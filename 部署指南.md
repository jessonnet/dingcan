...
ExecStart=/usr/bin/java -jar /var/www/canteen/backend/canteen-ordering-system-1.1.5.jar
Restart=always
RestartSec=10
...
# 克隆指定版本
git clone --branch V1.1.5 https://github.com/jessonnet/dingcan.git .
...
# 创建数据库
mysql -uroot -p123456 -e "CREATE DATABASE IF NOT EXISTS canteen_ordering_system CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
...
java -jar canteen-ordering-system-1.1.5.jar
```
...
2. **内存不足**：
   ...
   # 增加JVM内存参数
   # 修改ExecStart为：
   ExecStart=/usr/bin/java -Xmx2g -Xms1g -jar /var/www/canteen/backend/canteen-ordering-system-1.1.5.jar
   ```
...
# 修改ExecStart行为：
ExecStart=/usr/bin/java -Xmx2g -Xms1g -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -jar /var/www/canteen/backend/canteen-ordering-system-1.1.5.jar
...
/var/www/canteen/
├── backend/                    # 后端应用目录
│   ├── canteen-ordering-system-1.1.5.jar
│   └── application.yml
├── frontend/                   # 前端静态文件
│   └── dist/
│       ├── index.html
